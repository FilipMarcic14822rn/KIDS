	...
	public void run() {
		if (clientMessage.getMessageType() == MessageType.UPDATE) {
			if (clientMessage.getSenderPort() != AppConfig.myServentInfo.getListenerPort()) {
				ServentInfo newNodeInfo = new ServentInfo("localhost", clientMessage.getSenderPort(),AppConfig.myServentInfo.getSoftLimit(), AppConfig.myServentInfo.getHardLimit());
				...
				for (String port : ports) {
					allNodes.add(new ServentInfo("localhost", Integer.parseInt(port),AppConfig.myServentInfo.getSoftLimit(), AppConfig.myServentInfo.getHardLimit()));
				}
