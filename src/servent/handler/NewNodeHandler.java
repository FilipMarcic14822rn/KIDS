...
public class NewNodeHandler implements MessageHandler {
	...
	public void run() {
		if (clientMessage.getMessageType() == MessageType.NEW_NODE) {
			...
			ServentInfo newNodeInfo = new ServentInfo("localhost", newNodePort,AppConfig.myServentInfo.getSoftLimit(), AppConfig.myServentInfo.getHardLimit());
			...
			if (isMyPred) { //if yes, prepare and send welcome message
				...
				Map<Integer, List<ChordImage>> myValues = AppConfig.chordState.getValueMap();
				Map<Integer, List<ChordImage>> hisValues = new HashMap<>();
				
				int myId = AppConfig.myServentInfo.getChordId();
				int hisPredId = hisPred.getChordId();
				int newNodeId = newNodeInfo.getChordId();
				
				for (Entry<Integer, List<ChordImage>> valueEntry : myValues.entrySet()) {
